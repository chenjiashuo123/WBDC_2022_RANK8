...
def build_optimizer_clip_finetune(args, model_image, model_fc, num_total_steps):
    # Prepare optimizer and schedule (linear warmup and decay)
    no_decay = ['bias', 'LayerNorm.weight']
    optimizer_grouped_parameters = [
        {
        ...
        {
            "params": [p for n, p in model_fc.named_parameters() if not any(nd in n for nd in no_decay)],
            "weight_decay": args.weight_decay,
            "lr": args.other_learning_rate
        },
        {
            "params": [p for n, p in model_fc.named_parameters() if any(nd in n for nd in no_decay)], 
            "weight_decay": 0.0, 
            "lr": args.other_learning_rate
        }
    ]
