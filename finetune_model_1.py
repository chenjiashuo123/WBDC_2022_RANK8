...
from models.model_e2e import MULTIBERT
...
from dataset.dataset_e2e import create_dataloaders, create_dataloaders_aug, create_dataloaders
...
def train_and_validate(args):
    ...
    if args.full:
        ...
        logging.info(f'Load dataset len is {len(train_dataloader)}')
    else:
        train_dataloader, val_dataloader = create_dataloaders_aug(args)
        logging.info(f'Load dataset from {args.train_annotation}')
    # 2. build model and optimizers
    model = MULTIBERT(args)
    if args.pretrain_load_epoch != -1:
        checkpoint = torch.load(f'{args.pretrain_dir}/model_epoch_{args.pretrain_load_epoch}.bin', map_location='cpu')
        logging.info(f'Load pretrain weight from {args.pretrain_dir}/model_epoch_{args.pretrain_load_epoch}.bin')
        model.load_state_dict(checkpoint['model_state_dict'], strict=False)
    pretrained_model = None
    num_total_steps = len(train_dataloader) * args.max_epochs
    warmup_steps = int(num_total_steps*args.warmup_rate)
    optimizer, scheduler = build_optimizer(args, model, num_total_steps, pretrained_model, model_lr={'others':args.other_learning_rate, 'roberta':args.bert_learning_rate, 'visual_backbone':args.clip_learning_rate})
    ...
    for epoch in range(args.max_epochs):
        for batch in train_dataloader:
            ...
            if epoch>=1 and args.validation_steps > 0:
                if step % args.validation_steps == 0:
                    ...
                    torch.save({'epoch': epoch, 'model_state_dict': model.module.state_dict(), 'mean_f1': mean_f1},
                            f'{final_save_path}/model-step-{step}.bin')
